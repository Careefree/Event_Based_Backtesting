"""Backtest both long and short strategies."""
...
class BacktestLongShort(BacktestBase):
    ...
    def run_sma_strategy(self, SMA1, SMA2):
        ...
        print(msg)
        print("=" * 55)
        self.postition = 0 # inital neutral position
        ...
        for bar in range(SMA2, len(self.data)):
            if self.postition in [0, -1]:
                if self.data["SMA1"].iloc[bar] > self.data["SMA2"].iloc[bar]:
                    self.go_long(bar, amount="all")
                    self.postition = 1 # long position
            if self.postition in [0, 1]:
                if self.data["SMA1"].iloc[bar] < self.data["SMA2"].iloc[bar]:
                    self.go_short(bar, amount="all")
                    self.postition = -1 # short postition
        self.close_out(bar)
        ...
        for bar in range(momentum, len(self.data)):
            if self.position in [0, -1]:
                ...
                if self.position in [0, 1]:
                    if self.data["momentum"].iloc[bar] <= 0:
                        self.go_short(bar, amount="all")
                        self.position = -1 # short postition
        self.close_out(bar)
...
if __name__ == '__main__':
    ...
    lsbt = BacktestLongShort("2010-1-1", "2019-12-31", 10000, verbose=False)

    run_strategies()
    # transaction costs: 10 USD fix, 1% variable
    lsbt = BacktestLongShort("2010-1-1", "2019-12-31", 10000, 10.0, 0.01, False)
